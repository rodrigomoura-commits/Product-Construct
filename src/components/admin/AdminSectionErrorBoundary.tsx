...
export default class AdminSectionErrorBoundary extends React.Component<Props, State> {
  ...
  componentDidCatch(error: Error, info: React.ErrorInfo) {
    console.error("[AdminSectionErrorBoundary]", this.props.sectionName, error, info);
  }

  render() {
    if (this.state.hasError) {
      return (
        <div className="min-h-[520px] flex items-center justify-center p-8">
          <div className="max-w-2xl w-full bg-white border border-rose-100 rounded-[2rem] p-8 shadow-sm">
            ...
            <h2 className="text-xl font-black text-slate-900">
              Não consegui carregar esta seção do Admin.
            </h2>

            <p className="text-sm text-slate-500 mt-2">
              A seção {this.props.sectionName || "selecionada"} encontrou um erro de runtime.
              O restante do Admin continua disponível.
            </p>
